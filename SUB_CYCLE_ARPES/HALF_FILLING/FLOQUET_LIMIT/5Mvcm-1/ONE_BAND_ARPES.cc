...
// electronic
#define NN        612                                                   // # of k-points
#define t0        0.2
#define mu        0.4
#define BETA      100.0                      					     	// inverse temperature  300 K -> 40, 40 K --> 250

// Peierls driving
#define w_peierls      0.207            // ->50 THZ                                // Frequency of Applied Field (in eV)
#define Ax_peierls     0.927             // ->1 MV/cm                                 // Amplitude of Applied Field in x-direction
#define SIGMA          194.0
#define DELAY          380.0            //- > 100 fs @FWHM 

// FLOQUET
#define m_max 3	                                                        // m in {-m,...,0,...+m}
#define n_max 3                                                         // n in {-n,...,0,...+n}
#define timesteps 1e3                                                   // dt = 2.*M_PI/omega/double(timesteps)                                            

// Tr-ARPES
// probe pulse
#define starttime 0.0 
#define endtime   760.0 //- > 500 fs
#define TIMESTEPS 5e2                                                   // # of timesteps used for G<(t,t') -> timesteps/TIMESTEPS should be int!
#define T_PROBE 380.0   //- > 200 fs                                        		    // delay
#define SIGMA_PROBE 65.0   //- > 100 fs @FWHM                                     	    // RMS width
#define OMEGA_PROBE_MIN -1.0                                            // MINIMUM probe frequency
#define OMEGA_PROBE_MAX +0.5                                            // MAXIMUM probe frequency
#define N_OMEGA_PROBE 150                                               // # of probed frequencies
 ...
 */
{
	...
	{
		PROP_INTEGRAL[t+1] = PROP_INTEGRAL[t] + h*0.5*(set_Hk(BZ[k],double(t)*h) + set_Hk(BZ[k],double(t+1)*h));
	}	
	for(int t1=0; t1<TIMESTEPS-1; t1++)
	{
		for(int t2=0; t2<TIMESTEPS-1; t2++)
		{
			...
			G_Tr[fq_ARPES(t1,t2,k)] = II*fermi(set_Hk0(BZ[k]))*exp(-II*(PROP_INTEGRAL[t1]-PROP_INTEGRAL[t2]));
		}
	}		
}		 
		...
		for(int t=0; t<TIMESTEPS-1; t++)
		{
			if(gauss(double(t)*h, T_PROBE, SIGMA_PROBE)*gauss(double(tp)*h, T_PROBE, SIGMA_PROBE)*(2.*PI*pow(SIGMA_PROBE, 2))>weightcutoff)
			{
				...
				if (t==tp)	
					...
					shift = fmod(1./(h*double(t-tp))*(A_INTEGRAL[t]-A_INTEGRAL[tp]),2*PI);
				k_back_shift = k+int(round(shift/(2.*PI)*double(NN)));
				if (k_back_shift<0)
					k_back_shift += NN;
				else if (k_back_shift>NN-1)
					k_back_shift -= NN;	
				TEMP[tp] += 0.5*h*imag( gauss(double(t)*h, T_PROBE, SIGMA_PROBE)*gauss(double(tp)*h, T_PROBE, SIGMA_PROBE)*exp(II*omega*double(t-tp)*h)*G_Tr[fq_ARPES(t, tp, k_back_shift)]
									   +gauss(double(t+1)*h, T_PROBE, SIGMA_PROBE)*gauss(double(tp)*h, T_PROBE, SIGMA_PROBE)*exp(II*omega*double(t+1-tp)*h)*G_Tr[fq_ARPES(t+1, tp, k_back_shift)]);
			}
		}	  
	}	
	Iph = 0.0;	
...
int main(int argc, char * argv[])
{
	...
	for(int t=0; t<TIMESTEPS-1; t++)
	{
		A_INTEGRAL[t+1] = A_INTEGRAL[t] + h*0.5*(Ax_t(double(t)*h)+Ax_t(double(t+1)*h));
