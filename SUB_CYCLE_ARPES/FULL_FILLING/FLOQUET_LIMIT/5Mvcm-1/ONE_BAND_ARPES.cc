...
#define SIGMA          129.1			//- > 100 fs @FWHM 
#define DELAY          380.0            
	...
	for(int t2=0; t2<TIMESTEPS; t2++)
	{
		...
		for(int t1=0; t1<TIMESTEPS-1; t1++)
		{
			if(gauss(double(t1)*h, T_PROBE, SIGMA_PROBE)*gauss(double(t2)*h, T_PROBE, SIGMA_PROBE)*(2.*PI*pow(SIGMA_PROBE, 2))>weightcutoff)
			{
				...
				{
					shift1 = fmod(1./(h*double(t1-t2))*(A_INTEGRAL[t1*fac]-A_INTEGRAL[t2*fac]),2.*PI);
					//cout << "shift1: " << shift1 << " "; 
				}
				k_back_shift1 = k+int(round(shift1/(2.*PI)*double(NN)));
				if (k_back_shift1<0)
					k_back_shift1 += NN;
				else if (k_back_shift1>NN-1)
					k_back_shift1 -= NN;					
				if (t1+1==t2)	
				{
					shift2 = 0.0;
				...
				{
					shift2 = fmod(1./(h*double(t1+1-t2))*(A_INTEGRAL[t1*fac+1]-A_INTEGRAL[t2*fac]),2.*PI);
				}
				k_back_shift2 = k+int(round(shift2/(2.*PI)*double(NN)));
				if (k_back_shift2<0)
					k_back_shift2 += NN;
				else if (k_back_shift2>NN-1)
					k_back_shift2 -= NN;
				//if(k==64) cout << "k_shift: " << k_back_shift2 << endl;
