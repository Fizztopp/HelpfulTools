...
file_ARPES= open('DATA/ARPES.txt','r')
...
file_Driving = open('DATA/DRIVING_t.txt','r')
...
fit=np.polyfit(t/tc, Driving, 1, rcond=None, full=False, w=None, cov=False)
p = np.poly1d(fit)
pdx2 = np.polyder(p,1)
...
#ax01.set_yticks([0.0, -0.2, -0.4, -0.6, -0.8])
ax01.fill(t,-30+2*15*gauss(SIGMA_PROBE*tc, T_PROBE*tc, t), color="gainsboro", facecolor='gainsboro')
ax01.plot(t,p(t/tc), "k", linewidth=2.0, label=r"$\mathrm{E_{max}}= $"+str(Emax)+'$\mathrm{MV/cm}$')

#ax01.set_ylim(-0.81, +0.01)
...
ax00.plot(k,MAT_BANDS, 'k', linewidth=2.0)
ax00.vlines(x=0.0, ymin=omega_min, ymax = omega_max, color="k", linestyle="--", label="cut for EDC", linewidth=1.0)
