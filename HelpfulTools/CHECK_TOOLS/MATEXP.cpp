...
#define NATOM 10                                                      // Dimesnion of Matrix
#define ORDER 40                                                       // Order of Taylor Expansion

#ifndef NO_OMP
    #include <omp.h>
#endif

//#define TAYLOR_ONLY                                                   // Don't calculate matrixexponent by diaganoalization
//#define DIAG_ONLY                                                       // Don't calculate matrixexponent by Taylor Expansion
...
void ExpMat_Taylor(cvec &A, cvec *TEMP1, cvec *TEMP2, cvec &RESULT)
{
	...
	else
	{
		...
		for(int order=2; order<=ORDER; ++order)
		{
			...
			TEMP1 = temp2;
			TEMP2 = temp1;   
			//TEMP1[0] = TEMP2[0] COPIES all vlaues of vector!!!                         
			//cout << "fac: " <<  fac << endl;
#ifndef NO_OMP 		
			#pragma omp parallel for
#endif 						
...
#pragma omp parallel 
	...
	dtime1 = omp_get_wtime() - dtime;
	cout << "Calculations time of diagonalization(OMP): " << dtime1 << " seconds" << endl; 
	dtime0 = omp_get_wtime();
	for(int i=0; i<NATOM; i++)
	{
		for(int j=0; j<NATOM; j++)
		{
			(*TEMP1)[fq(i,j,NATOM)] = exp(evals[i])*delta(i,j);
		}
	}	
	times(TEMP1[0], MATRIX_COPY, TEMP2[0]);                                         // S @ H @ S^(-1) = H_D --> nk = S^(-1) @ nk_D @ S
	times_dn(MATRIX_COPY, TEMP2[0], RESULT1);	
	dtime1 = omp_get_wtime() - dtime0;
	dtime = omp_get_wtime() - dtime;
	cout << "Calculations time of backtransform (OMP): " << dtime1 << " seconds" << endl;
	cout << "Total calculations time (OMP): " << dtime << " seconds" << endl;
	//PRINT(RESULT1);
#endif
